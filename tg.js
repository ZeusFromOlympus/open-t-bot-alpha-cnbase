...
bot.action('PAY_FOR_SUBSCRIPTION', async (ctx) => {
  ...
  const sentMessage = await ctx.reply('Please enter your wallet address:', {
    reply_markup: {
      inline_keyboard: [
...
bot.command('start', async (ctx) => {
  const sentMessage = await ctx.replyWithPhoto({ source: 'src/https___editors.dexerto.com_wp-content_uploads_2023_03_14_OpenAI-GPT-4.jpg' }, {
    caption: `Your original caption here`,
    reply_markup: {
      inline_keyboard: [
...
bot.action('NEED_HELP', (ctx) => {
  ...
  bot.action('BACK', async (ctx) => {
    // Delete previous text message
    if (messageId) {
      await ctx.deleteMessage(messageId);
    }
 
    // Send original photo message
    const sentMessage = await ctx.replyWithPhoto({ source: 'src/https___editors.dexerto.com_wp-content_uploads_2023_03_14_OpenAI-GPT-4.jpg' }, {
      caption: `Your original caption here`,
      reply_markup: {
        inline_keyboard: [
